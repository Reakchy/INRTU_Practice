...
# 4. Ищем только внешние контуры (без всяких иерархий)
contours, _ = cv2.findContours(full_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

# 5. Если хоть один объект найден
if len(contours) > 0:
    ...
    # Считаем центр этой рамки (простой математикой)
    center_x = int(x + w / 2)
    center_y = int(y + h / 2)
    
    # Рисуем красный прямоугольник (0, 0, 255)
